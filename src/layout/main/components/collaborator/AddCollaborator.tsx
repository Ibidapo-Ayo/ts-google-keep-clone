...
const AddCollaborator = ({ noteId }: {
    ...
    const { openAction, tasks, setTasks, editorHandler, showTasks, setShowTasks } = useContext(GoogleKeepCloneContext)
    ...
    const handleSaveCollaborators = () => {
        const collaborator = showTasks.filter((task) => task.id === noteId)
